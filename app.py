# app.py — Final Clean Version (Robot Avatar + Voice + TTS + SHAP + PDF + Multi-language)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.info("Voice input supported via upload.\nSHAP + PDF included.\nPlacement ready 🚀")
...
with colB:
    ...
    <div class="card">
        ...
        <div class="small">Prediction • Diet • Exercise • Doctor • Multi-language • SHAP • PDF • Voice Assistant</div>
    </div>
    """, unsafe_allow_html=True)
...
def do_prediction():
    ...
    st.session_state["last_report"] = {
        ...
        "skin": skin_thickness, "insulin": insulin, "bmi": bmi,
        "dpf": dpf, "age": age, "prediction": "High" if pred == 1 else "Low",
        "probability": prob
    }
    return pred, prob, Xs
...
if "messages" not in st.session_state:
    st.session_state.messages = [
        {"role": "system", "content": f"You reply only in {lang}. Provide helpful but non-medical guidance."}
    ]

# Show chat history
for msg in st.session_state.messages[1:]:
    st.markdown(f"<div class='small'><b>{msg['role'].capitalize()}:</b> {msg['content']}</div>", unsafe_allow_html=True)

# User input
text_input = st.text_input("Ask your question:")

audio_input = st.file_uploader("Upload recorded voice:", type=["wav", "mp3", "m4a"])
...
if audio_input:
    trans = transcribe_audio(audio_input)
    final_user_msg = trans if trans else f"User uploaded audio file: {audio_input.name}"

if text_input.strip():
    final_user_msg = text_input

if st.button("Send to Assistant") and final_user_msg:
    ...
    if tts_enabled:
        try:
            ...
            tts.save(temp_mp3)
            st.audio(open(temp_mp3, "rb").read())
        except:
...
if st.button("Generate SHAP Plots"):
    if "last_report" not in st.session_state:
        ...
        for p in imgs:
            st.image(p)
        st.success("Explainability generated.")
...
if st.button("Download Report"):
    if "last_report" not in st.session_state:
        ...
        pdf.ln(4)
        pdf.cell(0, 8, "Diet/Exercise Tips below (use chatbot first):", ln=True)
...
# -------------------- FOOTER -------------------- #
st.markdown("<div class='small'>All features enabled: Robot avatar, voice, SHAP, PDF, multilanguage.</div>", unsafe_allow_html=True)
